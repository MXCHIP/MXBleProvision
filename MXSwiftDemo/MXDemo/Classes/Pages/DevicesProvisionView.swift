...
struct DevicesProvisionView: View {
    ...
    func startBleProvision(info: MXProvisionDeviceInfo) {
        ...
        let delegate = ProvisionDelegate { pk, identifier, error, dn in
        ...
        if let dn = info.deviceName, let pk = info.productKey {
            MXFogProvisionManager.shard.provisionDevice(peripheral: info.peripheral, productKey: pk, deviceName: dn, delegate: delegate)
        }
    }
...
// MARK: - 配网代理
class ProvisionDelegate: NSObject, MXBleProvisionDelegate {
    
    var requestNum: Int = 0
    
    public var provisionFinishHandler: (String, String, NSError?, String?) -> Void
    ...
    @objc func requestBleKey(params: [String : Any]?, type: Int, handler: @escaping (String?) -> Void) {
        ...
        if let dn = params?["device_name"] as? String,
           let pk = params?["product_key"] as? String {
            ...
            if let bleKey = MXFogProvisionManager.shard.createBleKey(secret: ds) {
                handler(bleKey)
            }
        }
    }
